...
NAME=$$(grep TerraformProviderName virtualizortf/version.go | grep -o -e 'terraform-provider-[a-z]*')
TARGETS=darwin linux windows
TERRAFORM_PLUGIN_EXTENSION=
VERSION=$$(grep TerraformProviderVersion virtualizortf/version.go | grep -o -e '[0-9]\.[0-9]\.[0-9]')
...
TERRAFORM_PLUGIN_DIRECTORY=$(TERRAFORM_CACHE_DIRECTORY)/terraform.danitso.com/provider/virtualizor/$(VERSION)/$(TERRAFORM_PLATFORM)
